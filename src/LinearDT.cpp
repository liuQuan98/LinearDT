    ...
    : range(*cloud)
{
	...
	auto center = Point3f((range.max.x + range.min.x) / 2, (range.max.y + range.min.y) / 2, (range.max.z + range.min.z) / 2);
...
float LinearDT::Evaluate(Point3f query) {
    ...
    Vector3f ratio;
    for (auto i = 0u; i < 3; i++) {
        coord[i] = std::min(IndexT(relPos[i] / cellEdge), nCells);
        ratio[i] = (relPos[i] - coord[i] * cellEdge) / cellEdge;
    }
